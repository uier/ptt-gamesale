...
const fetcher = async (page) => {
  const url = `https://store.playstation.com/zh-hant-tw/category/5f3aa730-1c8e-4a18-84da-030bb91558c0`
  const option = {
    headers: {
      'User-Agent': 'curl/8.1.2',
    },
  }
  return await fetch(`${url}/${page}`, option).then(res => res.text()).then(html => parse(html))
}
...
async function getPlayStationGames(start, end) {
  ...
  fs.writeFileSync(`./ps-games-${start}-${end}.json`, JSON.stringify(games, null, 2))
}
